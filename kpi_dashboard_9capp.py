...
# ---------------- Page + Branding ----------------
st.set_page_config(page_title="Modality Lewisham — A‑Team KPI Dashboard (v9c)", page_icon="🏥", layout="wide", initial_sidebar_state="expanded")
...
# ---------------- NEW: Unit‑level drill‑down (Option 1) ----------------
with st.expander("Klinik — drill down by Unit (for this staff + day)", expanded=False):
    unit_col = "unit"  # already in our combined df for Klinik rows
    # Allow optional hour filter within the day
    ...
    if klin_sel.empty:
        ...
        unit_tbl = (klin_sel[unit_col].dropna().astype(str).value_counts()
        ...
        csvu = unit_tbl.to_csv(index=False).encode("utf-8")
        st.download_button("Download unit breakdown (CSV)", data=csvu, file_name="unit_breakdown.csv", mime="text/csv")

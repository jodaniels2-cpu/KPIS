...
# ---------- Page + Branding ----------
st.set_page_config(page_title="Modality Lewisham — A‑Team KPI Dashboard (v9)", page_icon="🏥", layout="wide", initial_sidebar_state="expanded")

# NHS blue-ish theme accents via CSS
st.markdown(
    """
    <style>
    :root {
      --ml-primary: #005eb8; /* NHS Blue */
      --ml-accent: #00a3a3;
      --ml-muted: #f2f7ff;
    }
    .ml-header {
...
def password_gate():
    ...
    if not pw:
        st.error("No dashboard password is configured. Set **auth.password** in Secrets or **APP_PASSWORD** in environment variables.")
        st.stop()
    if st.session_state.get("authenticated", False):
        return True
    with st.form("login", clear_on_submit=False):
        st.subheader("🔒 Enter password to continue")
        entered = st.text_input("Password", type="password", help="Ask your admin for the dashboard password.")
...
def email_candidates(email):
    ...
    parts = re.split(r"[._+\-]", local)  # safe char class
    ...
    if parts:
        ...
        if last:
            keys.add(clean_token(first[:1] + last))  # j + smith
        keys.add(clean_token("".join(parts)))        # johnsmith
        keys.add(clean_token(local.replace(".", "").replace("_","").replace("-","").replace("+","")))  # jsmith
        initials = "".join(p[:1] for p in parts)
        keys.add(clean_token(initials))              # js
    else:
...
with st.sidebar:
    ...
    st.caption("Tip: you can drag-and-drop from Finder/Explorer.")
...
if doc_dt_col is None:
    doc_dt_col = find_datetime_col(doc_df, prefer_tokens=["date","time","event","completed"])
...
if call_start_col is None:
    call_start_col = find_datetime_col(call_df, prefer_tokens=["start","begin","created","call","time","date"])
...
case_staff_email = case_df[case_staff_col].map(norm_email_or_name)  # email preferred
...
# ---------- Calls events (Group vs Callback) ----------
...
# Staff source: callback => from Caller; else (group/inbound) => Answered/User
...
calls_ev = pd.DataFrame({"when": dt, "person": who_calls, "kind": calls_kind})
...
with st.sidebar:
    ...
    selected_user = st.selectbox("Staff Member", users, help="Names/emails from all sources are matched to a single person.")
...
if not week_df.empty:
    totals = {"Date":"Total",
              "Calls: Group": int(week_df["Calls: Group"].sum()),
...
with st.sidebar:
    pick_day = st.selectbox("Drill down day", [pd.Timestamp(d).strftime("%A %d %B") for d in valid_days], index=0 if default_day in valid_days else 0)
pick_day_dt = datetime.strptime(pick_day, "%A %d %B").date()

dr = udf[udf["date_only"]==pick_day_dt].copy()
dr["hour"] = dr["when"].dt.hour

hours = list(range(8,19))  # 8..18
rows = []
for h in hours:
    row = {"Hour": f"{h:02d}:00"}

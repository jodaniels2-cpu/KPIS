...
st.set_page_config(page_title="A‑Team KPI — Per‑User Hourly (v8b)", page_icon="🛠️", layout="wide", initial_sidebar_state="expanded")
...
def email_candidates(email):
    """Split the email's local part on dot/underscore/plus/hyphen safely."""
    if not isinstance(email, str): return set()
    local = email.split("@")[0].lower()
    parts = re.split(r"[._+\-]", local)  # FIX: safe character class
    ...
    if parts:
        ...
        keys.add(clean_token(local.replace(".", "").replace("_","").replace("-","").replace("+","")))
    ...
    for col in df.columns:
        ...
        if ser.dtype == "object" or "datetime" in str(ser.dtype).lower() or "date" in str(ser.dtype).lower() or "int" in str(ser.dtype).lower() or "float" in str(ser.dtype).lower():
            ...
            if ok >= 0.5:
                ...
                score = ok
                if any(tok in name_lower for tok in prefer_tokens):
                    score += 0.5
...
st.title("🛠️ Per‑User Hourly (v8b) — Calls + Klinik + Docman")
st.caption("Fixes email parsing regex and improves Calls Start Time detection.")
...
call_user_col    = find_col(call_cols, ["User Name"], contains_any=["user name","username","agent","owner","user"])
call_caller_col  = find_col(call_cols, ["Caller Name"], contains_any=["caller name","caller","callback","cb name"])
...
with st.expander("Diagnostics", expanded=False):
    ...
    st.write({
        ...
        "Calls":  {"user": call_user_col, "caller": call_caller_col, "outcome": call_outcome_col, "direction": call_direction_col, "start": call_start_col},
    })

...
st.set_page_config(page_title="Modality Lewisham — KPI Dashboard (v9d: 3 sections)", page_icon="🏥", layout="wide", initial_sidebar_state="expanded")
...
# ---------- Build per-source events & filters ----------
# Common date range
...
klinik = klinik[(klinik["when"]>=pd.Timestamp(start_dt)) & (klinik["when"]<=pd.Timestamp(end_dt))]
if clamp:
...
docman = docman[(docman["when"]>=pd.Timestamp(start_dt)) & (docman["when"]<=pd.Timestamp(end_dt))]
if clamp:
...
# Calls
call_dt = parse_dt(call_df[call_start_col], None)
type_lower = call_df[call_type_col].astype(str).str.lower()
is_callback = type_lower.str.contains("callback|call back|cb", na=False)
who = np.where(is_callback, call_df[call_caller_col].astype(str), call_df[call_user_answered_col].astype(str))
call_out_lower = call_df[call_outcome_col].astype(str).str.lower() if call_outcome_col else pd.Series([""]*len(call_df))
calls = pd.DataFrame({
    "when": call_dt,
    "person": pd.Series(who).map(normalise_name_or_email),
    "kind": np.where(is_callback, "Callback","Group")
}).dropna(subset=["when","person"])
if only_answered and call_outcome_col is not None:
    mask = call_out_lower.str.contains("answer|answered|connect|connected|complete|completed|handled|finished|resolved|success|ok", na=False)
    calls = calls[mask]
calls = calls[(calls["when"]>=pd.Timestamp(start_dt)) & (calls["when"]<=pd.Timestamp(end_dt))]
if clamp:
...
# ---------- UI: three independent sections ----------
st.markdown("## Klinik")
k_users = sorted(klinik["person"].dropna().unique().tolist())
...
if k_users:
    ...
    with st.expander("Units closed in (top)", expanded=False):
        if kdf.empty:
            st.info("No Klinik cases for this person in the selected range.")
        else:
...
st.markdown("## Docman")
d_users = sorted(docman["person"].dropna().unique().tolist())
...
st.markdown("## Calls")
c_users = sorted(calls["person"].dropna().unique().tolist())
c_user = st.selectbox("Calls staff member", c_users, key="call_picker", help="For callbacks, the 'person' is the caller name; for inbound, it's the 'User Name'.")
if c_users:

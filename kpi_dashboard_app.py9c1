...
# ---------------- Page + Branding ----------------
st.set_page_config(page_title="Modality Lewisham — A‑Team KPI Dashboard (v9c1)", page_icon="🏥", layout="wide", initial_sidebar_state="expanded")
...
def parse_dt(date_series, time_series=None):
    """Vectorised safe parser. If both date and time given, combine; else parse date field only.
       Includes a fallback when combined parse yields too many NaT.
    """
...
with st.sidebar:
    ...
    st.caption("Tip: drag‑and‑drop from Finder/Explorer.")
...
case_staff_col = find_col(case_cols, ["last_archived_by"], contains_any=["archived by","archived_by","staff","user","closed by"])
...
case_unit_col  = find_col(case_cols, ["unit_closed_in","unit"], contains_any=["unit","unit_closed","unit closed","closed in","closed_in"])
...
# Note: case_time_col can be None (date‑only files).
...
# Identity matcher (from Klinik emails)
...
with st.sidebar:
    ...
    selected_user = st.selectbox("Staff Member", users, help="All name/email variants are unified via Klinik emails.")

if not selected_user:
    st.info("Choose a staff member to continue."); st.stop()
...
for h in hours:
    row = {"Hour": f"{h:02d}:00"}
    sel = dr["hour"]==h
    row["Calls (Group+Callback)"] = int((((dr["kind"]=="Call — Group") | (dr["kind"]=="Call — Callback")) & sel).sum())
    row["Docman Completed"] = int(((dr["kind"]=="Docman Completed") & sel).sum())
    row["Klinik Case"] = int(((dr["kind"]=="Klinik Case") & sel).sum())
...
# ---------------- Unit‑level drill‑down (Option 1) ----------------
with st.expander("Klinik — drill down by Unit (for this staff + day)", expanded=False):
    hour_choice = st.selectbox("Hour filter (optional)", ["All hours"] + [f"{h:02d}:00" for h in hours], index=0)
...
# ---------------- Day details — robust grouping ----------------
c1,c2,c3 = st.columns(3)

# A) Calls — Group vs Callback (by hour) using pivot
with c1:
    st.caption("Calls — Group vs Callback (by hour)")
    calls_day = dr[dr["kind"].str.contains(r"^Call — ", regex=True, na=False)].copy()
    if calls_day.empty:
        st.info("No Calls for this staff/day.")
    else:
        calls_day["kind_clean"] = calls_day["kind"].str.replace("Call — ","", regex=False)
        pivot = (calls_day
                 .groupby(["hour","kind_clean"]).size()
                 .unstack(fill_value=0)
                 .reindex(index=hours, fill_value=0))
        for col in ["Group","Callback"]:
            if col not in pivot.columns:
                pivot[col] = 0
        calls_tbl = pivot[["Group","Callback"]].reset_index().rename(columns={"hour":"Hour"})
        calls_tbl["Hour"] = calls_tbl["Hour"].apply(lambda h: f"{h:02d}:00")
        calls_tbl["Total"] = calls_tbl["Group"] + calls_tbl["Callback"]
        st.dataframe(calls_tbl, use_container_width=True, height=280)

# B) Klinik — Units closed in (top for day)
with c2:
    ...
    if klin_day.empty:
        ...
        top_units = (klin_day["unit"].dropna().astype(str)
                     .value_counts().head(8).reset_index())
        top_units.columns = ["Unit","Cases"]
        st.dataframe(top_units, use_container_width=True, height=280)

# C) Docman — per hour using pivot
with c3:
    st.caption("Docman — per hour")
    doc_day = dr[dr["kind"]=="Docman Completed"]
    if doc_day.empty:
        st.info("No Docman tasks for this staff/day.")
    else:
        doc_pivot = (doc_day.groupby("hour").size()
                     .reindex(hours, fill_value=0).reset_index())
        doc_pivot.columns = ["Hour","Docman Completed"]
        doc_pivot["Hour"] = doc_pivot["Hour"].apply(lambda h: f"{h:02d}:00")
        st.dataframe(doc_pivot, use_container_width=True, height=280)

# ---------------- Diagnostics ----------------
with st.expander("Diagnostics (detected columns, matcher collisions)", expanded=False):
    st.write({
    ...
    st.write("Ambiguous keys ignored:", sorted(list(collisions))[:50])
